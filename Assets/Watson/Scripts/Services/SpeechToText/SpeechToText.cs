...
namespace IBM.Watson.DeveloperCloud.Services.SpeechToText.v1
{
  ...
  public class SpeechToText : IWatsonService
  {
    ...
    private void SendStart()
    {
	  ...
	  start ["keywords"] = this.Keywords; 
	  start ["keywords_threshold"] = this.KeywordsThreshold;

			string test = Json.Serialize (start);
    ...
    private SpeechRecognitionEvent ParseRecognizeResponse(IDictionary resp)
    {
      ...
      try
      {
        ...
        foreach (var r in iresults)
        {
		  ...
		  IDictionary<string,object> keywordsResult = iresult["keywords_result"] as IDictionary<string,object>;
		  ...
		  {
				foreach( var keywordResult in keywordsResult)
				{
					IList<object> listofResults = keywordResult.Value as IList<object>;
					if(listofResults != null)
					{
						foreach(var individualResult in listofResults)
						{
							IDictionary<string,object> auxResult = individualResult as IDictionary<string,object>;
							if(auxResult != null)
							{
								KeywordResult kResult = new KeywordResult();
								kResult.normalized_text = keywordResult.Key;
								kResult.confidence = (double)auxResult["confidence"];
								kResult.start_time = (double)auxResult["start_time"];
								kResult.end_time = (double)auxResult["end_time"];
								keywordResultList.Add(kResult);
							}
						}
					}
				}
		  ...
		  result.keywords_result.keyword = keywordResultList.ToArray();
