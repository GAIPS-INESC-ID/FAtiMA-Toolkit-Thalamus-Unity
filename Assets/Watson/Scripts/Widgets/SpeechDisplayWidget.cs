...
namespace IBM.Watson.DeveloperCloud.Widgets
{
  ...
  public class SpeechDisplayWidget : Widget
  {
	...
	[SerializeField]
	private Text m_KeywordsOutput = null;
    [SerializeField]
	...
	private string m_PreviousKeywordsOutputText = "";
      ...
      if (m_Output != null || m_OutputAsInputField != null)
      {
        ...
        if (result != null && result.results.Length > 0)
        {
          ...
          if (Time.time - m_TimeAtLastInterim > m_ThresholdTimeFromLastInput)
          {
			...
			if (m_KeywordsOutput != null) 
			{
				m_PreviousKeywordsOutputText = m_KeywordsOutput.text;
			}
		  ...
		  if (m_KeywordsOutput != null && m_ContinuousText)
		  {
			keywordsOutput = m_PreviousKeywordsOutputText;
		  }
          ...
          foreach (var res in result.results)
          {
			...
			if (res.final) 
			{
				foreach (KeywordResult kr in res.keywords_result.keyword) 
				{
					keywordsOutput += kr.normalized_text + ":" + kr.confidence + "\n";
				}

				if (m_KeywordsOutput != null) 
				{
					m_KeywordsOutput.text = keywordsOutput;
